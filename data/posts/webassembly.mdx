---
title: "WASM, WASI, W..hat? WebAssembly Everywhere"
date: June 2022
...
- **Software becomes much more extensible** - both through traditional plugin systems _and_ server-side compute of untrusted code. Of the above use cases, plugin systems, server-side compute of untrusted code and Inline Computation on everything likely blur into one similar use case.
- **Low Code/No Code takes off** (even more)
  - I could (and likely will) write a separate post on this. For now, take [Dynaboard](https://dynaboard.com) as the prime example of the power Wasm brings to low-code. By letting their users do things like create custom components and write client & server functions, they solve the graduation problem that most low-code tools have.
  - The biggest software companies become platforms. And platforms will use Wasm - so it makes sense the a company that owns this has a pretty massive market opportunity.
- W**rite once run anywhere** might actually become a thing
