...
So, Wasm let’s you write code in the language of your choice and run it anywhere. What makes it interesting?
...
There is some debate around the specifics of Wasm performance, but where it can excel is in allowing users to offload computationally intensive parts of their applications to lower-level languages.
...
> If VMs are the heavyweight class of cloud compute, and containers are the middleweight class, *WebAssembly is the perfect fit for a lightweight class*.
...
<Callout>
🧠 As Bailey Hayes at SingleStore put it in a recent talk at Kubecon (link coming soon!), a plugin can be much more than just an application. One could treat any smart contract as a plugin for a given blockchain, or any serverless function as a plugin. When one zooms out, many technologies can be treated like plugins.
...
Dynaboard uses WebAssembly for its plugin system. All user-supplied code runs in a Wasm sandbox. They chose WebAssembly over a few other options ([ShadowRealms](https://github.com/tc39/proposal-shadowrealm), Inline Frames on separate host with native JS eval(), a Custom DSL) because it is generally more performant, makes it simple to support multiple languages, and is easier to implement.
...
Shopify use WebAssembly to execute untrusted code in performance sensitive contexts (e.g. Checkout). This means that they can extend Shopify through Wasm in a safe and performant way.

#### _SecondState_

SecondState are building an edge-native PaaS powered by [WasmEdge](https://wasmedge.org/), their Wasm runtime built for cloud-native, edge, and decentralized applications. Alongside multiple languages, It runs JavaScript, including 3rd party ES6, CJS, and NPM modules.

#### _Fermyon_

Fermyon are also building an edge-native PaaS powered by WebAssembly, and generally, an ecosystem of tools that make for a best-in-class developer experience. I’m personally most excited about [Spin](https://github.com/fermyon/spin), **\*\***their framework for building and running fast, secure, and composable microservices.
...
For WebAssembly to thrive outside of the browser, it needs a standardized operating system to interface with whenever it runs outside of the browser, and all of the languages the run in WebAssembly [need](https://training.linuxfoundation.org/blog/wasi-bringing-webassembly-way-beyond-browsers/) a standard set of system calls (I/O interfaces) to communicate with that OS. This is what WASI (WebAssembly System Interface) is - it acts like a virtual operating system, making it possible to run the same Wasm binary across different operating systems, without compromosing the effectiveness of the WebAssembly sandbox. WASI is made up of a collection of standardized APIs. One could write an entirely separate piece on WASI, so I won’t cover it in any more detail here. [This website](https://wasmbyexample.dev/examples/wasi-introduction/wasi-introduction.all.en-us.html) gives a great overview of WASI, and [this repo](https://github.com/WebAssembly/WASI/blob/main/Proposals.md) tracks all the WASI APIs across three stages: Pre-Proposal, Feature Proposal, and Proposed Spec Text Available.

**The Component Model**

The WebAssembly component model’s high-level goals are describe [here](https://github.com/WebAssembly/component-model/blob/main/design/high-level/Goals.md). The first list goals is to:
...
In simpler terms, the aim of the component model is to eventually create a world where Wasm code can be used like [Lego Blocks](https://www.youtube.com/watch?v=NDe10MYs_c8)’. In the same way that developers can use NPM to install packaged JavaScript code, the Wasm component model aims to provide the building blocks to allow Wasm modules to become reusable. The cool thing here is, these Lego Blocks can be written in any language that compiles to Wasm, and in turn, can be imported into any language that compiles to Wasm. You can read more about how WASI works in relation to the component model [here](https://github.com/WebAssembly/component-model/blob/main/design/high-level/FAQ.md#how-does-wasi-relate-to-the-component-model).
...
- WASI & The Component model need to be fully integrated into the ecosystem in order for Wasm on the server to really take off
...
I wouldn’t exactly call any of these technologies direct alternatives to Wasm. All have their use cases and likely will for a long time, but they are the ones that kept popping up as I read this post so I felt it would be ammis not to mention them
...
I really like WebAssembly. You should also really like WebAssembly. It’s not without its flaws, and for the final time, I will say that some of what I wrote about above is theoretical, but it has the potential to revolutionize how we build applications. In a world where Wasm is ubiquitous (along with proposals such as WASI & The Component Model), applications will be safer, more performant, extensible. Not just traditional client/server web applications, but IoT, edge, and more. Historically, making things programmable has been a winning strategy (a la Cloudflare with their programmable CDN, AWS with programmable Cloud Infrastructure), and now Wasm makes that possible for a whole host of use cases.

I’ll leave you with some fairly tame & broad predictions. Unfortunately, as much as I am excited about Wasm, I’m not in the weeds every day figuring out how it could be used. If you are, please do share your thoughts [here](mailto:luke@tapestry.vc)!

- Polyglot applications become more common (especially thanks to WASI + The Component Model)
- Software becomes more extensible - both through traditional plugin systems _and_ server-side compute of untrusted code. Of the above use cases, plugin systems, server-side compute of untrusted code, and Inline Computation on everything likely blur into one similar use case.
- Write once run anywhere is within grasp
- There will be a de facto Wasm Package Registry
- There will be a universal debugger for Wasm, as well as better developer tools in general
- The browser gets one step closer to becoming the default operating system in large part due to applications porting to the web through using Wasm, as discussed earlier.
- Low Code/No Code takes off (even more)
  - I could (and likely will) write a separate post on this. For now, take Dynaboard as the prime example of the power Wasm brings to low-code. By letting their users do things like create custom components, write client & server functions, they solve the graduation problem that most low-code tools have.

## Reading List

All of the links to articles from the above are list below in chronological order, save some company website links. You can get a much more detailed (but poorly maintained by me) in the repo linked below. Please add cool links that you find! Else, [Awesome-wasm](https://github.com/mbasso/awesome-wasm) is a really great resource.

[GitHub - byrneml/wasm-projects: A curated list of projects, companies, and videos related to WebAssembly to document the interesting things I find as I explore the ecosystem.](https://github.com/byrneml/wasm-projects)

1. [https://webassembly.org/](https://webassembly.org/)
2. [https://nickymeuleman.netlify.app/blog/webassembly](https://nickymeuleman.netlify.app/blog/webassembly)
3. [https://harshal.sheth.io/2022/01/31/webassembly.html](https://harshal.sheth.io/2022/01/31/webassembly.html)
4. [https://zaplib.com/docs/blog_post_mortem.html](https://zaplib.com/docs/blog_post_mortem.html)
5. [https://www.fermyon.com/blog/how-to-think-about-wasm](https://www.fermyon.com/blog/how-to-think-about-wasm)
6. [https://www.figma.com/blog/webassembly-cut-figmas-load-time-by-3x/](https://www.figma.com/blog/webassembly-cut-figmas-load-time-by-3x/)
7. [https://github.com/appcypher/awesome-wasm-langs](https://github.com/appcypher/awesome-wasm-langs)
8. [https://en.wikipedia.org/wiki/Write_once,\_run_anywhere](https://en.wikipedia.org/wiki/Write_once,_run_anywhere)
9. [https://web.dev/ps-on-the-web/](https://web.dev/ps-on-the-web/)
10. [https://www.autodesk.com/products/autocad-web-app/overview](https://www.autodesk.com/products/autocad-web-app/overview)
11. [https://twitter.com/solomonstre/status/1111004913222324225](https://twitter.com/solomonstre/status/1111004913222324225)
12. [https://webassembly.org/docs/use-cases/](https://webassembly.org/docs/use-cases/)
13. [https://wasm.continuation-labs.com/d3demo/](https://wasm.continuation-labs.com/d3demo/)
14. [https://www.parity.io/blog/wasm-smart-contract-development/](https://www.parity.io/blog/wasm-smart-contract-development/)
15. [https://github.com/paritytech/wasmi](https://github.com/paritytech/wasmi)
16. [https://github.com/ewasm](https://github.com/ewasm)
17. [https://github.com/ewasm/design/blob/master/faq.md](https://github.com/ewasm/design/blob/master/faq.md)
18. [https://hackernoon.com/the-three-eggs-in-a-distributed-basket-wasm-blockchain-and-reputation-296892cdd77c](https://hackernoon.com/the-three-eggs-in-a-distributed-basket-wasm-blockchain-and-reputation-296892cdd77c)
19. [https://www.airtable.com/developers/scripting](https://www.airtable.com/developers/scripting)
20. [https://blog.cloudflare.com/workers-for-platforms/](https://blog.cloudflare.com/workers-for-platforms/)
21. [https://www.secondstate.io/faas/](https://www.secondstate.io/faas/)
22. [https://deno.com/deploy/docs](https://deno.com/deploy/docs)
23. [https://redpanda.com/](https://redpanda.com/)
24. [https://infinyon.com](https://infinyon.com)
25. [https://www.solo.io/blog/the-state-of-webassembly-in-envoy-proxy/](https://www.solo.io/blog/the-state-of-webassembly-in-envoy-proxy/)
26. [https://medium.com/wasm/ai-on-a-cloud-native-webassembly-runtime-wasmedge-part-i-3bf3714a64ea](https://medium.com/wasm/ai-on-a-cloud-native-webassembly-runtime-wasmedge-part-i-3bf3714a64ea)
27. [https://www.freecodecamp.org/news/jvm-tutorial-java-virtual-machine-architecture-explained-for-beginners/](https://www.freecodecamp.org/news/jvm-tutorial-java-virtual-machine-architecture-explained-for-beginners/)
28. [https://www.oracle.com/technical-resources/articles/java/architect-languages.html](https://www.oracle.com/technical-resources/articles/java/architect-languages.html)
29. [https://github.com/tc39/proposal-shadowrealm](https://github.com/tc39/proposal-shadowrealm)
30. [https://wasmedge.org/](https://wasmedge.org/)
31. [https://github.com/fermyon/spin](https://github.com/fermyon/spin)
32. [https://blog.stackblitz.com/posts/introducing-webcontainers/](https://blog.stackblitz.com/posts/introducing-webcontainers/)
33. [https://training.linuxfoundation.org/blog/wasi-bringing-webassembly-way-beyond-browsers/](https://training.linuxfoundation.org/blog/wasi-bringing-webassembly-way-beyond-browsers/)
34. [https://wasmbyexample.dev/examples/wasi-introduction/wasi-introduction.all.en-us.html](https://wasmbyexample.dev/examples/wasi-introduction/wasi-introduction.all.en-us.html)
35. [https://github.com/WebAssembly/WASI/blob/main/Proposals.md](https://github.com/WebAssembly/WASI/blob/main/Proposals.md)
36. [https://github.com/WebAssembly/WASI/blob/main/Proposals.md](https://github.com/WebAssembly/WASI/blob/main/Proposals.md)
37. [https://github.com/WebAssembly/component-model/blob/main/design/high-level/Goals.md](https://github.com/WebAssembly/component-model/blob/main/design/high-level/Goals.md)
38. [https://www.youtube.com/watch?v=Hq_cj77fuqo](https://www.youtube.com/watch?v=Hq_cj77fuqo)
39. [https://github.com/WebAssembly/component-model/blob/main/design/high-level/FAQ.md#how-does-wasi-relate-to-the-component-model](https://github.com/WebAssembly/component-model/blob/main/design/high-level/FAQ.md#how-does-wasi-relate-to-the-component-model)
40. [https://github.com/deislabs/wagi](https://github.com/deislabs/wagi)
41. [https://github.com/bytecodealliance/wit-bindgen/](https://github.com/bytecodealliance/wit-bindgen/)
42. [https://thenewstack.io/the-pain-of-debugging-webassembly/](https://thenewstack.io/the-pain-of-debugging-webassembly/)
43. [https://github.com/mbasso/awesome-wasm](https://github.com/mbasso/awesome-wasm)
